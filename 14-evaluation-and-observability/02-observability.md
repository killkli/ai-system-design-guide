...
- [追蹤 LLM 管道](#追蹤-llm-管道)
=======
# LLM 可觀測性（LLM Observability）

LLM 系統的可觀測性需要將日誌、指標和追蹤這三大支柱適應於 AI 應用的獨特特性。

## 目錄

- [為何 LLM 可觀測性不同](#為何-llm-可觀測性不同)
- [三大支柱](#三大支柱)
- [關鍵指標](#關鍵指標)
- [追蹤 LLM 管線](#追蹤-llm-管線)
>>>>>>> Stashed changes
...
=======
LLM 系統新增：
- **品質是一級指標**：一個快速、可用的系統但輸出惡劣，即是失敗
- **非確定性**：相同輸入可能產生不同輸出
- **Token 經濟學**：成本以複雜方式隨使用量擴展
- **多元件管線**：RAG 有檢索、重排序、生成步驟
- **主觀正確性**：通常沒有可比較的黃金標準
...
| 指標 | 說明 | 收集方式 |
|------|------|----------|
| 品質分數 | LLM 即評審評分 | 抽樣（1-5%） |
| 忠實度 | RAG 答案基於上下文 | 抽樣 |
| 相關性 | 答案回答問題 | 抽樣 |
| 使用者滿意度 | 贊/反對、評分 | 使用者回饋 |
| 任務完成 | 使用者達到目標了嗎？ | 隱含信號 |
...
| 每請求成本 | 平均成本 | 每模型 |
| 每日成本 | 總每日支出 | 整體 + 每模型 |
| 每使用者行動成本 | 完成使用者目標的成本 | 每任務類型 |
| Token 效率 | 每 Token 傳遞的價值 | 每用例 |
>>>>>>> Stashed changes
...
| 嚴重程度 | 回應時間 | 範例 |
|----------|----------|------|
| Critical | < 15 分鐘 | 服務停機、錯誤率 > 50% |
| High | < 1 小時 | 錯誤率 > 10%、P99 > 30s |
| Warning | < 4 小時 | 品質下降、成本飆升 |
...
- 請求率和錯誤率
- 延遲百分位：p50、p95、p99
- 流媒體的到第一個 Token 時間（TTFT）
- 可用性

**品質指標：** 這是 LLM 可觀測性的獨特之處。
- 使用 LLM 即評審的抽樣品質分數（1-5% 抽樣率）
- 對於 RAG：忠實度和相關性分數
- 使用者回饋：贊/反對、明確評分
- 可測量時的任務完成率
...
我為運營問題（錯誤率 > 5%、P95 > SLA）和品質漂移（平均分數從基線下降 10%）設定警報。成本警報有助於捕捉失控使用量。

關鍵見解是，一個快速、可用的 LLM 系統但輸出惡劣，仍然是失敗。品質必須是一級指標。」
...
**持續抽樣：** 我使用 LLM 即評審評估 1-5% 的請求。這給我一個品質信號，而不需評估一切。

**漂移偵測：** 我維護一個基線品質分佈，並使用統計測試來偵測當前分數是否顯著漂移。10% 的下降會觸發警告。

**使用者回饋：** 贊/反對、有明確評分的話。這是使用者滿意度的黃金標準。

**隱含信號：** 任務完成、重試率、升級率、會話長度。如果使用者更加掙扎，品質可能已經下降。

**當偵測到下降時該怎麼做：**
1. 檢查最近的部署或 prompt 變更
2. 抽樣特定回應以診斷問題
3. 檢查是否是模型特定的（提供者問題）或普遍的
4. 如有必要 rollback，然後調查

我還維護一個黃金測試集，包含具有預期行為的查詢，在每次部署時運行以在生產前捕捉回歸。」
>>>>>>> Stashed changes
...
=======
*下一篇：[CI/CD for LLM Applications](../11-infrastructure-and-mlops/02-cicd.md)*
>>>>>>> Stashed changes
